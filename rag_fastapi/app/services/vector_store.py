...
class VectorStore:
                ...
                if filename.startswith("index_") and filename.endswith(".faiss"):
                    ...
                    try:
                        ...
                        if os.path.exists(index_path) and os.path.exists(metadata_path):
                            ...
                            logger.info(f"Loaded FAISS index and metadata for job {job_id}.")
                        else:
                            logger.warning(f"Incomplete index files found for job {job_id}. Skipping load.")
                            # Clean up potentially orphaned files if one part exists but not the other
                            if os.path.exists(index_path): os.remove(index_path)
                            if os.path.exists(metadata_path): os.remove(metadata_path)

                    except Exception as e:
                        logger.error(f"Failed to load index for job {job_id}: {e}")
                        # Attempt to clean up corrupted files
    ...
    def _save_index(self, job_id: str):
        ...
        with self._lock: # RESTORED: with self._lock
            ...
            try:
                ...
                logger.debug(f"Job {job_id}: AFTER json.dump.")

                logger.info(f"FAISS index and metadata for job {job_id} saved successfully.")
            except Exception as e:
    ...
    def add_documents(self, job_id: str, chunks: List[DocumentChunk]) -> int:
        ...
        with self._lock:
            ...
            for i in range(0, len(chunks), settings.FAISS_BATCH_SIZE):
                ...
                self._indexes[job_id].add(embeddings_np) # RESTORED: faiss.add
    ...
    def search(self, job_id: str, query_embedding: List[float], k: int = 5) -> List[Tuple[float, DocumentChunk]]:
        ...
        with self._lock:
            if job_id not in self._indexes:
                logger.warning(f"No index found for job {job_id}. Cannot perform search.")
                return []
            ...
            for dist, idx in zip(distances[0], indices[0]):
                ...
                # Retrieve the original chunk metadata using the index in FAISS (which corresponds to addition order)
                # This assumes a direct mapping from FAISS internal ID to our chunk ID based on insertion order
                # For a more robust solution, map FAISS internal IDs to chunk_ids directly.
                # For simplicity here, we assume sequential addition and direct lookup.
                # A more robust approach would be to store a list of chunk_ids alongside the FAISS index.
                
                # Current simplistic approach: iterate through _chunk_metadata values, which isn't efficient for large scale
                # Let's improve this: when adding chunks, store (FAISS_internal_id, chunk_id) mapping
                
                # A more efficient way to map FAISS internal index to chunk_id is needed.
                # For now, let's just get all chunks for the job and map by FAISS add order.
                
                # A better way would be to store chunk_ids in a list corresponding to the order they were added to FAISS.
                if job_id not in self._chunk_metadata or not self._chunk_metadata[job_id]:
                    logger.error(f"Metadata not found for job {job_id}. Index might be corrupted or empty.")
                    return []

                # This is a temporary, inefficient workaround for mapping index to chunk.
                # In a real system, you'd store chunk_ids directly in the order they're added
                # or use an external mapping.
                all_job_chunks = list(self._chunk_metadata[job_id].values())
                if idx < len(all_job_chunks):
                    chunk = all_job_chunks[idx]
                    results.append((dist, chunk))
                else:
                    logger.warning(f"FAISS index {idx} out of bounds for metadata of job {job_id}. Data inconsistency.")
